    ...
    public class BlockHttpService : IBlockService
    {
        ...
        public async Task BlockUserAsync(Guid blockerUserId, Guid targetUserId)
        {
            var response = await _httpClient.PostAsync(
                $"api/Block?blockerUserId={blockerUserId}&targetUserId={targetUserId}", null);
        ...
        public async Task<bool> IsBlockedAsync(Guid blockerId, Guid targetId)
        {
            var response = await _httpClient.GetAsync(
                $"api/Block/check?blockerId={blockerId}&targetId={targetId}");
        ...
        {
            // The original service had two methods doing the exact same thing. 
            // We can just reuse the identical API call here to satisfy the Interface.
